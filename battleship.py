...
def makeModel(data):
    ...
    data['userboard'] = addShips(data['user'], data['userships'])  # Automatically place user ships
    data["tempship"]=[]
    data['winner']=None
    data['maxturns']=50
    data['minturns']=0
...
def mousePressed(data, event, board):
    ...
    if data['userships'] == 5 and board == "comp": #1
        runGameTurn(data, random.randint(0, 9), random.randint(0, 9), 'user')
...
def runGameTurn(data, row, col,player):
    ...
    #3
    user_guess = getComputerGuess(data['computerboard'])
    updateBoard(data, data['computerboard'], user_guess[0], user_guess[1], player='user')
